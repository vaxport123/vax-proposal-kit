...
## 헤더 (한준 2026-09-07 확정)
- **라벨 칩 + 절 제목**: 좌상단에 강조색 사각형 칩(모서리 4, 높이 34)에 흰 글자 18 Bold로 장 이름(「Ⅱ. 사업수행」·「목차」), 칩 오른쪽 16px에 절 제목 22 Bold. KITECH 손 덱의 「목차 INDEX」와 같은 문법이다.
- **헤더 아래 실선을 넣지 않는다.** 화면에서 얇은 가로줄이 「이상한 라인」으로 뜬다. 헤더와 제목은 여백(제목 y≈150)으로만 나눈다.
- **시그니처 사각형에 배경색을 칠하지 않는다**(`node.fills = []`). 흰 상자를 두면 배경이 회색·사진일 때 흰 덩어리가 보이고, 로고에도 흰 배경이 덧입혀진 것처럼 보인다. 이미지 채움(`upload_assets`)만 남긴다.
- 이미 만든 덱을 고칠 때: 실선(y≈112, 높이 1, 전폭 사각형) 제거 · 시그니처 사각형 `fills=[]` · 브레드크럼 텍스트 두 개 → 칩+절 제목으로 교체. `bids/순천캠퍼스VR/08_fix_header_v2.js`가 그 예다.

## 시그니처·사진
- 빈 사각형을 만들고 `upload_assets(nodeIds, scaleMode)`로 채운다: 시그니처 FIT(46곳 한 번에), 사진 FILL. URL마다 `curl -F file=@…` 한 번, 10분 안에. **시그니처 사각형은 배경 fills를 비워 두고** 이미지만 채운다(위 헤더 규칙).
