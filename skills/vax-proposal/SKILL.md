---
name: vax-proposal
description: 입찰 제안 하네스 — 「도전」 공고의 재료 팩(정량 지표·회사 자료·RFP 추출)을 위키에서 당겨와 P1 평가표 검산 → P2 가점 게이트 → P3 레퍼런스 → 방향·전략 → P4 초안+검토 → P5 심사 게이트(80점) → P6 슬라이드 계획 → Figma 까지 게이트 기반으로 진행한다. "공고번호 + 제안" 또는 "/bid-loop 사업폴더"로 실행. 직원이 「준비 점검」「새 사업 접수」「제안서 시작」「입찰 제안」「고스트 덱」「덱 만들어」「회고」라고 해도 이 스킬이다 — 첫 동작은 P-1 준비 점검, 그다음 P-0 접수(RFP·연결·방향·재료를 묻는다).
---
...
### P0 · 재료 팩
0. **담당자 제안 방식 읽기**(`references/personal-method.md`): 노션 「✍️ 제안 방식 — 개인별 커스텀」(데이터 소스 `e79e15a0-eb83-44fe-a517-3bf9fa751c62`)에서 **담당자 = 지금 사용자, 상태 = 사용중**인 페이지를 `notion-query-data-sources`로 찾아 본문 여섯 절(문체·방향 문구·검토 항목·참고 자료·피하는 표현·변경 기록)을 `_STATE.md` `개인방식` 칸에 페이지 ID와 함께 적는다. 없으면 「없음」을 적고 공통 규칙만으로 간다(막지 않는다). 개인 방식은 **공통 규칙 위에 얹는 취향**이다 — 게이트·근거·공개 금지·재료 팩 계약과 부딪히면 공통 규칙이 이기고, 그 사실을 사용자에게 한 줄 알린다. 작업 중 사용자가 새 규칙을 정하면(「앞으로 ~하게 써」) 그 사람 페이지의 해당 절 끝에 덧붙이고 변경 기록 표에 한 줄 넣는다 — 지우지 않는다.
