...
### P0 · 재료 팩
0. **담당자 제안 방식 — 사용자는 아무것도 켜고 끄지 않는다**(`references/personal-method.md`): 노션 「✍️ 제안 방식 — 개인별 커스텀」(데이터 소스 `e79e15a0-eb83-44fe-a517-3bf9fa751c62`)에서 **담당자 = 지금 사용자**(`notion-fetch self`로 확인)인 페이지를 `notion-query-data-sources`로 찾아 본문 여섯 절(문체·방향 문구·검토 항목·참고 자료·피하는 표현·변경 기록)을 읽고 `_STATE.md` `개인방식` 칸에 페이지 ID와 함께 적는다. **없으면 Claude가 그 자리에서 만든다** — 「제안 방식 — {이름}」, 담당자 = 사용자, 상태 = 사용중, 본문은 여섯 절 제목만(예시 없이). 한 줄만 알린다: 「제안 방식 페이지를 만들었습니다. 작업하며 정하는 규칙은 여기에 자동으로 쌓입니다」. 상태가 「보류」인 페이지만 읽지 않는다. 개인 방식은 **공통 규칙 위에 얹는 취향**이다 — 게이트·근거·공개 금지·재료 팩 계약과 부딪히면 공통 규칙이 이기고, 그 사실을 사용자에게 한 줄 알린다. 작업 중 사용자가 규칙을 정하면(「앞으로 ~하게 써」「이 표현 쓰지 마」「이 덱 참고해」) **묻지 않고** 그 사람 페이지의 해당 절 끝에 덧붙이고 변경 기록 표에 한 줄 넣는다 — 지우지 않는다. 한 공고에서 여러 번 정했으면 P5 통과 시점에 한 번 「이번에 저장한 규칙 n개」로 알린다.
