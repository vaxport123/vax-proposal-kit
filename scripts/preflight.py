...
판정 항목(필수 = 없으면 P0로 못 간다):
  필수  파이썬 3.10+ · 스킬 설치본이 저장소와 같음 · 비판자 에이전트 설치 · 파이썬 의존성(ddgs·requests·bs4·PIL) · 셀프테스트 6종
...
def checks(online=False):
    ...
    fails = []
    for f in ("ui_tokens", "render_html", "figures_svg", "proposal_lint", "layout_wireframes", "probe_web"):
        rc, _ = run([sys.executable, os.path.join(SCRIPTS, f + ".py"), "--selftest"], timeout=90)
        if rc != 0:
            fails.append(f)
    add("selftest", True, not fails, "셀프테스트 6종 " + ("통과" if not fails else "실패: " + ",".join(fails)),
